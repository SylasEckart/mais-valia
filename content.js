...
const
    horasPorDia = 8
const minutoSalario = (function(){
    const
        salario = 6200,
        horasSemanais = 40,
        totalSemanas = 5
    return (salario / (horasSemanais * totalSemanas) ) / 60
})
...
const detectaSufixo = (function(sufixo) {
    switch (sufixo) {
        case "mil":
            sufixo_numero = 1000;
            break;
...
const tempoSalario = (function(valor_final) {
    console.log({valor_final})
    if(!isNaN(valor_final)){
        tempo_salario = (valor_final / minutoSalario());
        console.log({tempo_salario},{valor_final})
        ...
        if (tempo_salario < 1) {
            ...
            if (tempo_salario == 1)
                tempo_salario_sufixo = 'minuto';
        }
        total_calculado = tempo_salario.replace(".", ",") + " " + tempo_salario_sufixo + ' de trabalho';
        return total_calculado;    
    }
})

const substituiValores = (function(evt) {
                        ...
                        for (var k = 0; k < total_preco.length - 1; k++) {
                            ...
                            if (valores[0] == ' ') {
                                ...
                                if (typeof tempo_salario !== 'undefined' && !isNaN(valor_financeiro)) {
                                    console.log("Valor "+string_a_substituir+" substituído por "+total_calculado);
...
jQuery(document).ready(function(){
    console.log({razao:window.razao})
    substituiValores('*');
});
